...
const NewArrival = () => {
  ...
  useEffect(() => {
    ...
    if (adminFromStorage) setAdmin(JSON.parse(adminFromStorage));
  }, []);

  useEffect(() => {
    const fetchNewArrivalVehicles = async () => {
      try {
        setLoading(true);
        const { data } = await axios.get(`${BACKEND_URL}/api/vehicles`);
        const newArrivals = data.filter(vehicle => vehicle.isNewArrival);
        setVehicles(newArrivals);
        setLoading(false);
      } catch (err) {
        setError('Failed to fetch new arrival vehicles');
        toast.error('Failed to fetch vehicles');
        setLoading(false);
      }
    };

    fetchNewArrivalVehicles();
  }, [BACKEND_URL]);
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-b from-gray-50 to-gray-100 px-4">
        <div className="bg-white p-8 rounded-lg shadow-lg border border-amber-200 w-full max-w-md text-center">
          ...
          <p className="text-gray-600 mb-8">Please log in or register to view our new arrivals.</p>
      ...
      <div className="container mx-auto px-4 py-8 md:py-12">
        <div className="text-center mb-10">
          ...
          <p className="text-gray-600 max-w-2xl mx-auto">
            Discover the latest additions to our premium fleet
          </p>
        </div>
        ...
        {vehicles.length === 0 ? (
          <div className="bg-white p-10 rounded-lg shadow-md text-center max-w-2xl mx-auto border border-amber-200">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 text-amber-400 mx-auto mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 9 0 0118 0z" />
            </svg>
            <p className="text-xl text-gray-700 mb-4">No new arrival vehicles found</p>
            <p className="text-gray-500 mb-6">Stay tuned! More vehicles will be added soon.</p>
          </div>
        ) : (
